...
def validate_birth_year(year):
	...
	if year < 1900:
		...
		raise ValueError(f'Birth year "{year}" must be earlier than 2019')
	return year

def calculate_retirement_age(birth_year):
	birth_year = validate_birth_year(birth_year)
...
def calculate_retirement_date(birth_year, birth_month, age_years, age_months):
	birth_year = validate_birth_year(birth_year)
	birth_month = validate_birth_month(birth_month)
	age_years = validate_age_year(age_years)
	age_months = validate_age_month(age_months)
